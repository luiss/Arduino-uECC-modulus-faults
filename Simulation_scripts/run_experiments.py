...
def main():
    ...
    parser.add_argument("--fault-model", choices=["00", "FF"], required=True,
    ...
    lists = build_factor_lists(curve, fault_model, args.B)
    ...
    for target_entropy in range(args.min_entropy, max_entropy + args.step, args.step):
        ...
        if fault_type == PERSISTENT:
            ...
            if res:
                print(f"PFA -> H={target_entropy:4d} | faults={len(res[0]):3d} | entropy={res[2]:8.2f} | cost={res[3]}")
        else:
