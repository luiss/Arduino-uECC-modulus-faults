...
#include <Arduino.h>
#include <avr/io.h>
extern void c_log(const char* message);
extern void c_log_hex(uint8_t* buffer, size_t length);
extern void signal_high();
extern void signal_low();

extern char __data_start;       // Start of .data in SRAM
extern char __data_load_start;  // Start of .data in Flash (PROGMEM)
...
// Reloads the modulus q (curve->n) from Flash to SRAM
// using the exact instruction sequence of the AVR boot copy routine.

static void reload_modulus_exact(uint8_t *dest_sram, const uint8_t *src_flash, uint16_t num_bytes) {
    uint8_t *end_sram = dest_sram + num_bytes;

    asm volatile (
        "rjmp 2f \n"
        "1: \n"
        "lpm r0, Z+ \n"              // <--- Loads byte from Flash (Z) into r0
        "st X+, r0 \n"               // <--- Stores byte from r0 to SRAM (X)
        "2: \n"
        "cp %A[dest], %A[end] \n"
        "cpc %B[dest], %B[end] \n"
        "brne 1b \n"   
...
static int uECC_sign_with_k_internal(const uint8_t *private_key,
    ...
    // curve secp256
    // Dynamically compute the Flash address value to copy modulus from
    uint16_t data_offset = (uint16_t)(curve->n) - (uint16_t)&__data_start;
    const uint8_t *flash_modulus_n = (const uint8_t *)(&__data_load_start + data_offset);

    uint8_t k_size = 32;

    signal_high(); // trigger start
    // FA Modif <--


    reload_modulus_exact((uint8_t *)curve->n, (const uint8_t *)flash_modulus_n, // Source array in PROGMEM/Flash
     curve->num_bytes);

    // FA Modif -->
    signal_low(); // trigger stop
    // FA Modif <--
